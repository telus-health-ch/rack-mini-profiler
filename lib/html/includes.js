...
var _MiniProfiler = (function() {
  ...
  var allowedRequestURL = function allowedRequestURL(responseURL) {
    if (!options.allowedRequestUrlPattern) {
      return responseURL.indexOf(window.location.origin) !== 0;
    }
    return options.allowedRequestUrlPattern.test(responseURL);
  };
    ...
    if (!window.MiniProfiler || !window.MiniProfiler.patchesApplied) {
      ...
      XMLHttpRequest.prototype.send = function(data) {
        ...
        this.addEventListener("load", function() {
          ...
          if (this.responseURL && !allowedRequestURL(this.responseURL)) {
            return;
          }
  ...
  return {
    ...
    init: function init() {
      ...
      this.options = options = (function() {
        ...
        var allowedRequestUrlPattern = script
          .getAttribute("data-request-url-pattern");
        if (allowedRequestUrlPattern) allowedRequestUrlPattern =
          new RegExp(allowedRequestUrlPattern);
        return {
          ids: ids,
          path: path,
          version: version,
          ...
          allowedRequestUrlPattern: allowedRequestUrlPattern
        };
      })();
    ...
    moreUrl: function moreUrl(requestUrl) {
      var linkSuffix = requestUrl.indexOf("?") > 0 ? "&pp=help" : "?pp=help";
      return requestUrl + linkSuffix;
    },
